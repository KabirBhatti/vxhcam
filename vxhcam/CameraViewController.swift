...
//  vxhcam
//
//  Created by Kabir on 19/03/2024.
//
...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, AVCapturePhotoOutputReadinessCoordinatorDelegate {
    ...
    var currentSelectedIndexPath: Int?
    var lastSelectedIndexPath: Int = 2
    ...
    override func viewDidLoad() {
        ...
        let indexPath = IndexPath(item: 2, section: 0)
            ...
            if let selectedCell = collectionView.cellForItem(at: indexPath) as? CaptureModeCollectionViewCell {
                selectedCell.modeLbl.textColor = UIColor.yellow
            }
    }
    ...
    var modes = ["TIME-LAPSE", "SLOW-MO", "PHOTO", "VIDEO", "PORTRAIT", "CINEMATIC", ]
    
    func isModeEnable(){
        if currentSelectedIndexPath == 2 {
        ...
        } else if currentSelectedIndexPath == 3 {
...
//        captureModeControl.isEnabled = false
        ...
        self.changeCamera(nil, isUserSelection: true, completion: {
            ...
            DispatchQueue.main.async {
                ...
                self.photoButton.isEnabled = self.movieFileOutput != nil
                self.photoButton.isEnabled = true
...
//                self.captureModeControl.isEnabled = true
    ...
    private func setUpPhotoSettings() -> AVCapturePhotoSettings {
        ...
        if let device = captureDevice {
                   do {
                       try device.lockForConfiguration()
                       // Set desired zoom factor (1.0 = no zoom, up to device.activeFormat.videoMaxZoomFactor)
                       device.videoZoomFactor = 2.0 // Example zoom factor
                       device.unlockForConfiguration()
                   } catch {
...
//    }
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didStartRecordingTo fileURL: URL, from connections: [AVCaptureConnection]) {
        ...
        DispatchQueue.main.async {
            self.photoButton.isEnabled = true
            self.photoButton.setImage(#imageLiteral(resourceName: "CaptureStop"), for: [])
        }
    }
    ...
    func fileOutput(_ output: AVCaptureFileOutput,
        ...
        DispatchQueue.main.async {
            ...
            self.photoButton.isEnabled = true
//            self.captureModeControl.isEnabled = true
            self.photoButton.setImage(#imageLiteral(resourceName: "Shutter"), for: [])
    ...
    private func addObservers() {
        let keyValueObservation = session.observe(\.isRunning, options: .new) { _, change in
            ...
            DispatchQueue.main.async {
                ...
                self.photoButton.isEnabled = isSessionRunning && self.movieFileOutput != nil
...
extension CameraViewController{
    func registerNib() {
        let nib = UINib(nibName: CaptureModeCollectionViewCell.nibName, bundle: nil)
        collectionView?.register(nib, forCellWithReuseIdentifier: CaptureModeCollectionViewCell.reuseIdentifier)
...
extension CameraViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CaptureModeCollectionViewCell.reuseIdentifier,
                                                         for: indexPath) as? CaptureModeCollectionViewCell {
            let name = modes[indexPath.row]
...
extension CameraViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView,
        ...
        guard let cell: CaptureModeCollectionViewCell = Bundle.main.loadNibNamed(CaptureModeCollectionViewCell.nibName,
                                                                      owner: self,
                                                                      options: nil)?.first as? CaptureModeCollectionViewCell else {
            return CGSize.zero
        }
        cell.configureCell(name: modes[indexPath.row])
        cell.setNeedsLayout()
        cell.layoutIfNeeded()
...
extension CameraViewController: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        // Select the current cell and update its appearance
        collectionView.selectItem(at: indexPath, animated: true, scrollPosition: .centeredHorizontally)
        if let selectedCell = collectionView.cellForItem(at: indexPath) as? CaptureModeCollectionViewCell {
            selectedCell.modeLbl.textColor = UIColor.yellow
            self.lastSelectedIndexPath = currentSelectedIndexPath ?? 2
            self.currentSelectedIndexPath = indexPath.row
            print("selected index:: \(String(describing: currentSelectedIndexPath))")
            print("lastSelectedIndexPath: \(String(describing: lastSelectedIndexPath))")
            isModeEnable()
        }
        // Deselect the previously selected cell and reset its appearance
        if let deselectedCell = collectionView.cellForItem(at: IndexPath(item: self.lastSelectedIndexPath, section: 0)) as? CaptureModeCollectionViewCell {
            deselectedCell.modeLbl.textColor = UIColor.white // Reset the text color to white
        }
    }
}
